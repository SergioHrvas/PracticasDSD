...
public class Donaciones extends UnicastRemoteObject implements iDonaciones, SDonaciones, Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
               ...
               /* for(int i = 0; i < 3; i++){
                    System.out.println("| L | Tiene Token Replica " + i + " :" +((SDonaciones)reg.lookup("Replica"+i)).getTieneToken());
            }*/
    ...
    public void solicitar() throws AccessException, RemoteException, NotBoundException {
        System.out.println("| S | Esta Operando Replica " + n_replica + " :" +((SDonaciones)reg.lookup("Replica"+n_replica)).getEstaOperando());
        System.out.println("| S | Tiene Token Replica " + n_replica + " :" +((SDonaciones)reg.lookup("Replica"+n_replica)).getTieneToken());
        boolean tiene = ((SDonaciones)reg.lookup("Replica"+n_replica)).getTieneToken();
        while (tiene == false) {
    ...
    public void liberar() throws RemoteException, NotBoundException {
        ...
        System.out.println("| L | Esta Operando Replica " + n_replica + " :" +((SDonaciones)reg.lookup("Replica"+n_replica)).getEstaOperando());
        System.out.println("| L | Tiene Token Replica " + n_replica + " :" +((SDonaciones)reg.lookup("Replica"+n_replica)).getTieneToken());
